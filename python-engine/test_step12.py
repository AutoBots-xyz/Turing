...
if __name__ == "__main__":
    ...
    for idx, em in enumerate(extracted_mechanisms, 1):
        ...
        print("  Edges (Verbs):")
        for edge in em.causal_graph.edges:
            src_label = next((n.label for n in em.causal_graph.nodes if n.id == edge.source), edge.source)
            tgt_label = next((n.label for n in em.causal_graph.nodes if n.id == edge.target), edge.target)
